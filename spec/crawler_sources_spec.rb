...
# as source text instead, the way relaton-data-iana guards its own.
RSpec.describe "crawler.rb" do
  ...
  # Relaton::Etsi::DataFetcher writes index-v2 only. index-v1 is rebuilt from
  # the data/ tree the fetch just produced, so it has to run after it.
  it "builds index-v1 after the fetch" do
    fetch = source.index("Relaton::Etsi::DataFetcher.fetch")
    build = source.index("EtsiIndexBuilder.build_index_v1")
